...
const FAQ = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto xl:px-0 px-5 mb-20 font-andika">
      <h2 className="text-center pt-5  text-4xl font-bold  text-[#101a30] mb-5">
        FAQ
      </h2>
      <p className="text-xl text-center text-brand-color font-semibold mb-10 capitalize lg:mb-20">
      ...
      {dataLoaing ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 sm:grid-cols-1">
          {data?.map((item) => (
            <div
              key={item?.id}
              className="collapse collapse-plus rounded-lg border border-brand-color text-[#101a30]"
            >
