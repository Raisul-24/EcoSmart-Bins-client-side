...
const MyCart = () => {
  ...
  const handlePayment = () => {
    axiosPublic
      .post("/cart-order", { totalPrice, email: user?.email })
      .then((res) => {
          ...
          {isLoading ? (
            ...
            <div className="relative flex flex-col w-full h-full overflow-scroll text-green-950 bg-transparent shadow-md rounded-xl bg-clip-border">
              <table className="w-full text-left table-auto min-w-max">
                <thead className="text-center bg-lime-100">
                  <tr>
                    <th className="p-4 border-b border-blue-gray-100 bg-blue-gray-50">
                      <p className="block font-sans text-sm antialiased leading-none text-green-950 opacity-70">
                        Image
                      </p>
                    </th>
                    <th className="p-4 border-b border-blue-gray-100 bg-blue-gray-50">
                ...
                <tbody className="text-center">
                  {cart.map((item) => (
                    <tr key={item._id}>
                      ...
                      <td className="p-4 border-b border-blue-gray-50">
                        <p className="block font-sans font-bold text-sm antialiased leading-normal text-blue-gray-900">
                          {item.price} Tk
                        </p>
                      </td>
                    </tr>
              ...
              <div className="flex justify-end m-4 ">
                <Btn onClick={handlePayment}>Order Now</Btn>
              </div>
            </div>
          )}
        </div>
